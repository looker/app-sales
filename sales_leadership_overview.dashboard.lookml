- dashboard: sales_leadership_quarter_overview
  ...
  - title: Quota Attainment
    ...
    fields: [opportunity.day_of_quarter, opportunity.total_closed_won_new_business_amount,
      quota_numbers.quarterly_aggregate_quota_measure, opportunity.close_quarter]
    ...
    dynamic_fields: [{table_calculation: running_total_of_amount, label: Running Total
      ...
      {table_calculation: percent_of_quota, label: Percent of Quota, expression: "${running_total_of_amount}/${quota_numbers.quarterly_aggregate_quota_measure}",
  ...
  - title: of Quota
    ...
    dynamic_fields: [{table_calculation: of_quota_hit, label: "% of Quota Hit", expression: "${opportunity.total_closed_won_new_business_amount}\
          \ / ${quota_numbers.quarterly_aggregate_quota_measure}", value_format: !!null '',
  ...
  - title: Rep Performance Overview
    ...
    limit: 500
    column_limit: 50
    dynamic_fields: [{table_calculation: to_quota, label: "% to Quota", expression: "${opportunity.total_new_closed_won_amount_qtd}/${quota_numbers.quarterly_quota}",
        value_format: !!null '', value_format_name: percent_0, _kind_hint: measure,
        _type_hint: number}, {table_calculation: gap, label: Gap, expression: 'if((${quota_numbers.quarterly_quota}-${opportunity.total_new_closed_won_amount_qtd})>0,${quota_numbers.quarterly_quota}-${opportunity.total_new_closed_won_amount_qtd},0)',
        value_format: !!null '', value_format_name: usd_0, _kind_hint: measure, _type_hint: number},
  ...
  - title: Performance vs Quota
    ...
    explore: opportunity
    type: looker_line
    fields: [opportunity.close_date, quota_numbers.quarterly_aggregate_quota_measure,
    ...
    dynamic_fields: [{table_calculation: quota_per_day, label: Quota Per Day, expression: "${quota_as_table_calc}/count(row())",
      ...
      {table_calculation: cumulative_quota, label: Cumulative Quota, expression: 'running_total(${quota_per_day})
        ...
        value_format: '[>=1000000]$0.00,,"M";[>=1000]$0.00,"K";$0.00', value_format_name: !!null '',
        _kind_hint: measure, _type_hint: number}, {table_calculation: grab_quota_value,
        label: Grab Quota Value, expression: "# Used to account for the fact that\
          \ the measure might be null for dates that are dimensino-filled in.\n# We\
          \ take the max of an offset list of our quuarterly agg quota measure column,\
          \ so as long as at least one row has a non-dimension filled close date,\
          \ we can grab the quota value\n\nif(row() = 1, max(offset_list(${quota_numbers.quarterly_aggregate_quota_measure},0,91)),null)",
        ...
        value_format_name: !!null '', _kind_hint: measure, _type_hint: number}, {
        table_calculation: is_before_today, label: Is before today, expression: "${opportunity.close_date}\
