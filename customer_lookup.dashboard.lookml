- dashboard: customer_lookup
  ...
  - title: Days As Customer
    ...
    type: single_value
    fields:
    - account.days_as_customer
    - account.name
    sorts:
    - account.days_as_customer desc
  ...
  - name: Days To Close
    ...
    - model: sales_analytics
      ...
      - account.business_segment
      sorts:
      - opportunity.average_days_to_closed_won desc
      limit: 500
      column_limit: 50
      total: true
      query_timezone: America/Los_Angeles
    ...
    hidden_fields:
    - account.business_segment
    - q1_opportunity.average_days_to_closed_won
    total: true
    dynamic_fields:
    - table_calculation: compared_to_avg
      label: Compared to Avg
      expression: |-
        (${q1_opportunity.average_days_to_closed_won:total} - ${opportunity.average_days_to_closed_won:total})/
        ${opportunity.average_days_to_closed_won:total}
      value_format:
  ...
  - title: Account History
    ...
    fields:
    - opportunity.name
    - opportunity_owner.created_date
    - opportunity.close_date
    - opportunity.source
    - opportunity_owner.name
    - opportunity.total_closed_won_amount
    sorts:
    - opportunity_owner.created_date desc
    limit: 500
    query_timezone: America/Los_Angeles
    show_view_names: false
    series_types: {}
  ...
  - name: CLV
    ...
    - model: sales_analytics
      ...
      - account.business_segment
      sorts:
      - opportunity.average_amount_won desc
      limit: 500
      total: true
      query_timezone: America/Los_Angeles
      join_fields:
  ...
  - name: Days in Stages
    ...
    - model: sales_analytics
      ...
      filters:
        opportunity_stage_history.stage: "-NULL"
      sorts:
      - opportunity_stage_history.avg_days_in_stage desc
      limit: 500
      query_timezone: America/Los_Angeles
      join_fields:
    ...
    - label: ''
      orientation: left
      series:
      - id: opportunity_stage_history.avg_days_in_stage
        name: Days In Stage
        axisId: opportunity_stage_history.avg_days_in_stage
      - id: q1_opportunity_stage_history.avg_days_in_stage
        name: Average (All Accounts)
        axisId: q1_opportunity_stage_history.avg_days_in_stage
      showLabels: false
      showValues: false
      unpinAxis: false
      tickDensity: default
  ...
  - name: Revenue
    ...
    hidden_fields:
    - account.business_segment
    - q1_account.average_annual_revenue
    series_types: {}
    total: true
    dynamic_fields:
    - table_calculation: average_for_account
      label: Average for account
      expression: "${opportunity.average_amount_won:total}"
      value_format: '[>=1000000]$0.00,,"M";[>=1000]$0.00,"K";$0.00'
      value_format_name:
      _kind_hint: measure
      _type_hint: number
    - table_calculation: compared_to_avg_for_all
      label: Compared to Avg for all
      expression: "(${opportunity.average_amount_won} - ${q1_opportunity.average_amount_won})/\
        \ ${q1_opportunity.average_amount_won}"
      value_format:
      value_format_name: percent_1
      _kind_hint: measure
      _type_hint: number
